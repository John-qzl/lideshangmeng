...
public class OrderDetailsAdapter2 extends BaseRecyclerAdapter<OrderSubmit.ListBean.GoodsListBean, OrderDetailsAdapter2.OrderDetalisHolder> {
    ...
    @Override
    public void onBindViewHolder(final OrderDetalisHolder holder, final OrderSubmit.ListBean.GoodsListBean data, final int position) {
        ...
        MyImageLoader.getInstance().loadImageReturnBitMap(
                holder.ivImg, data.getGoods_thumb(), 0

        );
        holder.tvName.setText(data.getGoods_name());
        holder.tvPrice.setText("¥ " + data.getGoods_number() + "");

...
public class YesPaySingleFragment extends BaseFragment implements MineSingleContract.SingleList, PullToRefreshBase.OnRefreshListener2<ListView> {
    ...
    private PullToRefreshListView lv_yessingle;
    private int page = 1;
    ...
    protected int getLayoutId() {
        return R.layout.fragment_yespaysingle;
    }

    @Override
    protected void init() {
        lv_yessingle = (PullToRefreshListView) getRootView().findViewById(R.id.lv_yessingle);
        lv_yessingle.setMode(PullToRefreshBase.Mode.PULL_FROM_END);
        lv_yessingle.setOnRefreshListener(this);
        mineDoSingleApater = new MineDoSingleApater(getContext());
        mineDoSingleApater.setDataList(singleList);
        lv_yessingle.setAdapter(mineDoSingleApater);
        if (presenter == null) {
    ...
    protected void lazyLoad() {
        if (App.getApp().isPaySingleYesSuccess()) {
            if (presenter == null) {
                presenter = new MineSinglePresenter(this);
            }
            page = 1;
            isClear = true;
            presenter.getSingleList();
            App.getApp().setPaySingleYesSuccess(false);
        }
    }

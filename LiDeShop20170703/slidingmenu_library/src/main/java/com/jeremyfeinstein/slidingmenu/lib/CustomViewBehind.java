...
public class CustomViewBehind extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		mContent.layout(0, 0, width - mWidthOffset, height);   //设置左边侧边栏的宽度 高度

		if (mSecondaryContent != null){
			//对child调用其layout方法，来安排child的大小和位置。
			mSecondaryContent.layout(0, 0, width - mWidthOffset, height);  //设置右边侧边栏的宽度 高度
	...
	private int mMode;
	private boolean mFadeEnabled;        //SlidingMenu构造方法初始化为真 开启
	private final Paint mFadePaint = new Paint();
	private float mScrollScale;
	private Drawable mShadowDrawable;
	private Drawable mSecondaryShadowDrawable;
	private int mShadowWidth;
	private float mFadeDegree;    //侧滑view 淡出淡入，就是从界面从黑到透明
	...
	/**
	 * 绘制侧滑View的淡出淡入
	 * @param content 要绘制的View
	 * @param canvas  画布
	 * @param openPercent 打开了的百分比
	 */
	public void drawFade(View content, Canvas canvas, float openPercent) {
		if (!mFadeEnabled) return;   //不开启就返回，不设置 淡入淡出效果
